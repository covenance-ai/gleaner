#!/usr/bin/env python3
"""Gleaner backfill: upload all existing Claude Code session transcripts.

Scans ~/.claude/projects/ for JSONL session files and uploads any that
haven't been uploaded yet (checks with the server first).

Usage:
    python3 backfill.py                    # upload all sessions
    python3 backfill.py --dry-run          # just list what would be uploaded
    python3 backfill.py --project foo      # only sessions from project "foo"
...
# Reuse the upload machinery from the hook script
sys.path.insert(0, str(Path(__file__).parent))
from upload_session_lib import (
    collect_provenance,
    parse_transcript,
    upload,
)
...
def get_existing_session_ids() -> set[str]:
    ...
    if not GLEANER_URL or not GLEANER_TOKEN:
        return set()
    try:
        url = f"{GLEANER_URL.rstrip('/')}/api/sessions?ids_only=true"
